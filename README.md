...
| TP_C | 멀티모달 AI 웹툰/동화 창작 플랫폼 | LangChain 멀티 에이전트, 이미지 생성, 일관성 검증 | [`AI_Native_TP_Multimodal_Webtoon/`](./AI_Native_TP_Multimodal_Webtoon/) |
...
> **TP_C_멀티모달 웹툰**: 체험형 데모 웹앱 (API 키 불필요, 로컬 실행). 수채화풍 동화 "숲속의 기사 토끼" 4컷 포함.
> 샘플 이미지(`AIN_TP_sample_images.zip`)는 아래 Google Drive에서 다운로드 후 `sample_images/` 폴더에 압축 해제하세요.
> [Google Drive 링크](https://drive.google.com/drive/folders/1-Eqv09FmsdJ7ywOZag8wg-zHomjIOGKs?usp=drive_link)
> ```bash
> cd AI_Native_TP_Multimodal_Webtoon
> npm create vite@latest webtoon-demo -- --template react
> cd webtoon-demo
> copy "..\demo.jsx" src\App.jsx
> xcopy "..\sample_images" public\sample_images\ /E
> npm install
> npm run dev
> ```
...
└── AI_Native_TP_Multimodal_Webtoon/
    ├── README.md                    # 프로젝트 개요 + 로컬 실행 방법
    ├── demo.jsx                     # 체험형 데모 웹앱 (API 키 불필요)
    ├── core_agents.py               # LangChain 멀티 에이전트 핵심 코드
    └── sample_images/               # Gemini 생성 동화 4컷 이미지
```
